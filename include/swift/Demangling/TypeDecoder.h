...
void decodeRequirement(
    NodePointer node, std::vector<BuiltRequirement> &requirements,
    BuilderType &Builder, std::function<BuiltType(NodePointer)> decodeTypeFn,
    std::function<BuiltLayoutConstraint(LayoutConstraintKind)>
        getLayoutConstraintFn,
    std::function<BuiltLayoutConstraint(
        LayoutConstraintKind, unsigned SizeInBits, unsigned Alignment)>
        getLayoutConstraintWithSizeAlignFn) {
  for (auto &child : *node) {
    ...
    if (child->getNumChildren() != 2)
      return;
    auto subjectType = decodeTypeFn(child->getChild(0));
    if (!subjectType)
      return;
    ...
    if (child->getKind() ==
      ...
      constraintType = decodeTypeFn(child->getChild(1));
      if (!constraintType)
        return;
    }
    ...
    case Demangle::Node::Kind::DependentGenericLayoutRequirement: {
      ...
      if (kind != LayoutConstraintKind::TrivialOfExactSize &&
        ...
        layout = getLayoutConstraintFn(*kind);
      } else {
        ...
        layout = getLayoutConstraintWithSizeAlignFn(*kind, size, alignment);
      }
...
public:
  ...
  TypeLookupErrorOr<BuiltType> decodeMangledType(NodePointer Node) {
    ...
    case NodeKind::SILBoxTypeWithLayout: {
      ...
      std::vector<BuiltSubstitution> substitutions;
      std::vector<BuiltRequirement> requirements;
      ...
      if (Node->getNumChildren() > 1) {
        ...
        if (dependentGenericSignatureNode->getNumChildren() < 1)
          return MAKE_NODE_TYPE_ERROR(
              ...
              "fewer children (%u) than required (1)",
